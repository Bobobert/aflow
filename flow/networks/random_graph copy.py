...
class RandomGridNetwork(Network):
    ...
    * **speed_limit** : speed limit for all edges. This may be represented as a
    ...
    def _start_(self):
        ...
        rotations = [0, 1, 2, 3]
        # Generating the grid
        grid_cells = self.rg.choice(type_of_cell,\
        ...
        for i in range(self.n_row):
            for j in range(self.n_col):
                edge_pad = grid_edges[i,j]
                for k, edge in enumerate(edge_pad):
                    if edge:
                        from_node = ((2*j+1)+(2*i+1)*(self.n_col*2+1)) #Center node
                        if k == 0:
                            self.edges += new_edge(from_node,(2*j+2)+(2*i+1)*(self.n_col*2+1))
                        elif k == 1:
                            self.edges += new_edge(from_node,(2*j+1)+(2*i)*(self.n_col*2+1))
                        elif k == 2:
                            self.edges += new_edge(from_node,(2*j)+(2*i+1)*(self.n_col*2+1))
                        elif k == 3:
                            self.edges += new_edge(from_node,(2*j+1)+(2*i+2)*(self.n_col*2+1))
